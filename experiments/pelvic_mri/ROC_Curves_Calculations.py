...
DEFAULT_ROC_INPUT_PATHS: Tuple[Path, ...] = (
    Path("/home/mluser1/Musti_Anomaly_Detection/Complete_AnomalyDetection_LUND-PROBE/Inference_Results_LUND_PROBE_Christian_Clinical/results_v4_zscore.json"),
...
# Canonical grouping for all-anomaly stratified tables.
# The tokens are matched against patient_id/category/case_folder text.
...
def main() -> None:
    ...
    patient_summary = payload.get("patient_summary", [])
    if not patient_summary:
        logging.info("patient_summary not found or empty; aggregating from slice-level results")
    ...
    # Only generate the combined TokenSurprisal + BinarySum figure and unique anomaly patient counter.
    try:
        if not results:
        ...
        logging.warning("Skipping combined token/binary and unique-counter plots: %s", exc)
    return
