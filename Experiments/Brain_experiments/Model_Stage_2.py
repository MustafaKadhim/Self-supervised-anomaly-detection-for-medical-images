...
class RotaryEmbedding2D(nn.Module):
    """2D Rotary Position Embeddings for slice-wise tokens.

    Splits head_dim into row and column parts only.
    """
    ...
    def __init__(
        ...
        self.dim = dim
        self.half_dim = (dim // 2) // 2 * 2
        self.row_dim = self.half_dim
        self.col_dim = dim - self.half_dim
...
class TransformerBlockSDPA(nn.Module):
    """Transformer block using explicit F.scaled_dot_product_attention with 2D RoPE."""
    ...
    def forward(
        ...
        """Forward pass (slice position ignored; using 2D RoPE).
        
        Args:
            x: (B, S, D) input tokens
            slice_pos: Optional slice indices (ignored).
        """
        B, S, D = x.shape
...
class TransformerSDPA(nn.Module):
    """Stack of TransformerBlockSDPA layers with 2D positional encoding."""
    ...
    def forward(
        ...
        """Forward pass (slice position ignored).
...
class FactorizedMaskGIT(pl.LightningModule):
    def __init__(
        ...
        # AYNU: state for training/alt-scoring (kept in __init__ for checkpoint compatibility)
    ...
    # =========================================================================
    # AYNU — auxiliary methods of FactorizedMaskGIT (training, alt-scoring, …)
    # =========================================================================
    ...
    def _apply_mask(self, tokens: torch.Tensor, mask_token_id: int, task: str) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        # Vectorized to avoid per-sample syncs
        mask_counts = mask.sum(dim=-1)
        empty_indices = (mask_counts == 0).nonzero(as_tuple=False).flatten()
        if empty_indices.numel() > 0:
            rand_pos = torch.randint(0, mask.size(1), (empty_indices.numel(),), device=mask.device)
            mask[empty_indices, rand_pos] = True
    ...
    def _apply_block_mask(self, tokens: torch.Tensor, mask_token_id: int, task: str) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        rate = self._sample_rate(task) if self.training else self.mask_ratio
        # Heuristic: generate multiple random blocks in parallel and union them
        # to approximate the target coverage without while-loops or .item() syncs.
        num_blocks = int(max(1, 16 * rate * 1.5))

        top = torch.randint(0, max(1, H - 1), (B, num_blocks), device=device)
        left = torch.randint(0, max(1, W - 1), (B, num_blocks), device=device)
        h_sizes = torch.randint(2, max(3, H // 2), (B, num_blocks), device=device)
        w_sizes = torch.randint(2, max(3, W // 2), (B, num_blocks), device=device)

        grid_r, grid_c = torch.meshgrid(
            torch.arange(H, device=device),
            torch.arange(W, device=device),
            indexing="ij",
        )
        grid_r = grid_r.unsqueeze(0).unsqueeze(0)
        grid_c = grid_c.unsqueeze(0).unsqueeze(0)

        top = top.unsqueeze(-1).unsqueeze(-1)
        left = left.unsqueeze(-1).unsqueeze(-1)
        h_sizes = h_sizes.unsqueeze(-1).unsqueeze(-1)
        w_sizes = w_sizes.unsqueeze(-1).unsqueeze(-1)

        in_block = (
            (grid_r >= top) & (grid_r < top + h_sizes) &
            (grid_c >= left) & (grid_c < left + w_sizes)
        )
        mask = in_block.any(dim=1).view(B, S)

        # Ensure at least one masked token per sample
        mask_counts = mask.sum(dim=-1)
        empty_indices = (mask_counts == 0).nonzero(as_tuple=False).flatten()
        if empty_indices.numel() > 0:
            rand_pos = torch.randint(0, S, (empty_indices.numel(),), device=device)
            mask[empty_indices, rand_pos] = True
        masked_tokens = tokens.masked_fill(mask, mask_token_id)
        return masked_tokens, mask
    ...
    def _masked_ce(logits: torch.Tensor, target: torch.Tensor, mask: torch.Tensor, label_smoothing: float = 0.0) -> torch.Tensor:
        # Avoid CPU syncs from .sum() or boolean checks by using a masked reduction
        ...
        masked_loss_sum = (loss_all * flat_mask.float()).sum()
        total_masked = flat_mask.sum()
        return masked_loss_sum / total_masked.clamp(min=1.0)
    ...
    def training_step(self, batch, batch_idx):
        ...
        logits_l1, target_l1, mask_l1 = self(indices, task="l1", use_block_mask=use_block, slice_pos=None)
        logits_l2, target_l2, mask_l2 = self(indices, task="l2", use_block_mask=use_block, slice_pos=None)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        logits_l1, target_l1, mask_l1 = self(indices, task="l1", use_block_mask=False, slice_pos=None)
        logits_l2, target_l2, mask_l2 = self(indices, task="l2", use_block_mask=False, slice_pos=None)
        ...
        hidden_center = self.transformer(tokens_center, slice_pos=None)
    ...
    def compute_anomaly_map(
        ...
        hidden = self.transformer(tokens_anat, slice_pos=None)
        ...
        hidden_tex = self.transformer(tokens_tex, slice_pos=None)
        ...
        for row_start in range(0, h - window_size + 1, stride):
            for col_start in range(0, w - window_size + 1, stride):
                ...
                for _ in range(num_monte_carlo):
                    ...
                    hidden_l1 = self.transformer(tokens_l1, slice_pos=None)
                    ...
                    hidden_l2 = self.transformer(tokens_l2, slice_pos=None)
    ...
    def compute_anomaly_map_iterative(
        ...
        _, probs_l1 = self.iterative_predict(
            indices, task="l1", num_steps=num_steps, initial_mask_ratio=initial_mask_ratio, slice_pos=None
        )
        _, probs_l2 = self.iterative_predict(
            indices, task="l2", num_steps=num_steps, initial_mask_ratio=initial_mask_ratio, slice_pos=None
        )
    ...
    def iterative_predict(
        ...
        for _ in range(num_steps):
            ...
            hidden = self.transformer(tokens, slice_pos=None)
            logits = head(hidden)
        ...
        hidden_final = self.transformer(final_tokens, slice_pos=None)
