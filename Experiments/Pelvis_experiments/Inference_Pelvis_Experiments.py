...
def recursive_automask_v4_zscore(
    ...
    # Preprocessing
    # ---------------------------------------------------------
    # AYNU: sharpness score and map populate the JSON `artifact_flag` /
    # `sharpness_score` fields and the artifact-guard branch below.
    # They do not affect the iteration-0 heatmap or mask used by AUROC.
    ...
    # AYNU: stage1 reconstruction and LPIPS(input, recon) feed the
    # `lpips_input_recon_sum_mask` JSON field and visualisations only.
    ...
    for iter_idx in range(num_iterations):
        ...
        # Targeted Inpainting
        # ---------------------------------------------------------
        # AYNU: targeted_inpaint output is consumed only by iteration > 0.
        # With num_iterations=1 (the AUROC default) it is unused.
...
def main():
    ...
    # CORE: healing and TTA parameters
    ...
    # CORE: inference mode for AUROC JSON generation.
    print("\n" + "="*70)
    ...
    # AYNU: patient_summary is retained for diagnostic plots; AUROC recomputes patient scores from per-slice CORE fields.
    ...
    # AYNU: convert diagnostic patient summary to a stable, readable order.
    patient_summary = []
