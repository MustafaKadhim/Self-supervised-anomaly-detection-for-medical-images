...
6. binary mask fusion: both use ALM-A ∪ ALM-B only (no LPIPS backflow in either);
7. post-fusion edge erosion: Brain applies edge-aware erosion to the final binary mask; Pelvis does not;
8. cohort-label policies for ROC;
9. Pelvis explicitly reporting PR/AUPRC in the main merged ROC workflow.
